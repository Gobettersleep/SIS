<template>
  <div class="grades-page">
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card"><div class="stat-icon" style="background:#d1fae5;color:#059669;"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></div><div class="stat-info"><span class="stat-value">{{ filteredGrades.length }}</span><span class="stat-label">总记录数</span></div></div>
    </div>
...
const stats = computed(() => { const list = filteredGrades.value; if (list.length === 0) return { average: '-', highest: '-', lowest: '-' }; const scores = list.map(g => Number(g.score)); const avg = (scores.reduce((a, b) => a + b, 0) / scores.length).toFixed(1); return { average: avg, highest: Math.max(...scores), lowest: Math.min(...scores) } })

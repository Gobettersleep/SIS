      ...
      <div v-if="showForm" class="modal-overlay" @click.self="closeForm">
        <div class="modal-content" role="dialog" aria-modal="true">
          ...
          <form @submit.prevent="handleSubmit" class="modal-form">
            ...
            <div class="form-group" :class="{ 'form-group--error': formErrors.teacher }"><label class="form-label">授课教师<span class="required">*</span></label><input type="text" v-model="form.teacher" placeholder="请输入教师姓名" @blur="validateField('teacher')" /><p v-if="formErrors.teacher" class="error-text">{{ formErrors.teacher }}</p></div>
...
const editCourse = (c) => { isEditing.value = true; form.value = { ...c }; formErrors.value = {}; showForm.value = true }
...
.card-actions { display: flex; gap: 0.5rem; padding-top: 0.75rem; border-top: 1px solid var(--border-color); }
...
.modal-content { background-color: #ffffff; border-radius: 0.75rem; width: 100%; max-width: 480px; max-height: 90vh; overflow-y: auto; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); border: 1px solid #e0ddd8; } .modal-content--small { max-width: 400px; }
